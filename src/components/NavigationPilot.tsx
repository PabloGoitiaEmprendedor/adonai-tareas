...
export function NavigationPilot({ menuItems, settingsItems, showAdmin, electronOffset, user, onNavigate }: NavigationPilotProps) {
  ...
  const totalBadge = menuItems.reduce((total, item) => total + (item.badge || 0), 0);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onMouseLeave={hide}
        className={cn(
          "pointer-events-auto absolute left-0 top-1/2 -translate-y-1/2 w-[24px] h-24 rounded-r-2xl transition-all duration-200 flex items-center justify-center",
          open
            ? "bg-white shadow-2xl opacity-100"
            : "bg-white/95 backdrop-blur-md opacity-100 hover:opacity-100",
          "shadow-[0_16px_34px_rgba(15,23,42,0.30),0_4px_14px_rgba(91,124,250,0.20)]"
        )}
      >
        {totalBadge > 0 && (
          <span className="absolute -right-2 -top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-black leading-none text-white shadow-lg shadow-red-500/35">
            {totalBadge > 99 ? '99+' : totalBadge}
          </span>
        )}
        <div className="flex flex-col items-center gap-1">
          <div className="w-[4px] h-[4px] rounded-full bg-slate-500/70" />
          <div className="w-[4px] h-[4px] rounded-full bg-slate-500/70" />
          <div className="w-[4px] h-[4px] rounded-full bg-slate-500/70" />
        </div>
      </div>
