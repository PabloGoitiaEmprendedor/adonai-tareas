...
export const useStreaks = () => {
 ...
 queryFn: async () => {
 if (!user) return null;
 const { data, error } = await supabase.from('experiment_metrics').select('*').eq('user_id', user.id).maybeSingle();
 ...
 const { data: freshMetrics, error: fetchError } = await supabase.from('experiment_metrics').select('*').eq('user_id', user.id).maybeSingle();
 ...
 const { data: initialized, error: initError } = await supabase.from('experiment_metrics').insert({
 user_id: user.id,
 ...
 const { error: updateError } = await supabase.from('experiment_metrics').update(updates).eq('user_id', user.id);
 ...
 const { data: existing } = await supabase.from('user_achievements').select('achievement_id, achievements(code)').eq('user_id', user.id);
 const have = new Set((existing || []).map((e: any) => e.achievements?.code));
 const toUnlock = streakCodes.filter(c =>!have.has(c));
 if (toUnlock.length > 0) {
 const { data: catalog } = await supabase.from('achievements').select('*').in('code', toUnlock);
