...
const PriorityHighlighter = ({ content }: { content: string }) => {
 ...
 const isCompleted = priorityInstruction?.includes('Dominado') ?? false;
 ...
 boxShadow: isCompleted? '0 0 15px rgba(195, 245, 60, 0.15)': ['0 0 12px rgba(239, 68, 68, 0.08)', '0 0 22px rgba(239, 68, 68, 0.18)', '0 0 12px rgba(239, 68, 68, 0.08)']
 }}
 transition={{
 ...
 isCompleted? 'bg-primary/10 border-primary/20': 'bg-red-500/10 border-red-500/30'
 }`}
 >
 ...
 canProceed? 'bg-primary text-black shadow-lg shadow-primary/20 opacity-100 cursor-pointer active:scale-[0.97]': 'bg-white/10 text-white/30 border border-white/5 opacity-50 cursor-default'
 }`}
 >
...
const AppTutorial = ({ run, onFinish }: AppTutorialProps) => {
 ...
 /* Aseguramos que el overlay de Joyride sea el que reciba cualquier clic residual */.react-joyride__spotlight {
 pointer-events: none!important;
 }
 `;
 }
