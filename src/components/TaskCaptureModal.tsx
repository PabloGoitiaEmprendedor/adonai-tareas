...
const TaskCaptureModal = forwardRef<TaskCaptureModalHandle, TaskCaptureModalProps>(({ open, onClose, goalId, folderId, timeBlockId, initialMode, creationSource }, ref) => {
 ...
 selectedGoalId === goal.id? "bg-primary text-primary-foreground border-primary shadow-lg shadow-primary/20": "bg-surface text-muted-foreground border-outline-variant hover:bg-on-surface/5"
 )}
 >
 ...
 isMini? "h-14": "h-16",
 reviewImportance? "bg-amber-500/20 text-amber-600 border-amber-500/50 shadow-lg shadow-amber-500/10": "bg-surface text-muted-foreground border-outline-variant hover:bg-on-surface/5"
 )}
 >
 IMPORTANTE
 </button>
 ...
 "flex flex-col items-center justify-center gap-1 rounded-[22px] font-black uppercase tracking-widest text-[9px] transition-all border",
 isMini? "h-14": "h-16",
 reviewUrgency? "bg-red-500/20 text-red-600 border-red-500/50 shadow-lg shadow-red-500/10": "bg-surface text-muted-foreground border-outline-variant hover:bg-on-surface/5"
 )}
 >
 URGENTE
 </button>
