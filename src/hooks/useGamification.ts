...
export const useGamification = () => {
 ...
 const { data: metrics } = await supabase.from('experiment_metrics').select('*').eq('user_id', user.id).maybeSingle();
 if (!metrics) return;
 ...
 await supabase.from('experiment_metrics').update({
 ...
 const { data: catalog } = await supabase.from('achievements').select('*').in('code', newCodes);
 ...
 const { data: m2 } = await supabase.from('experiment_metrics').select('xp_total, level').eq('user_id', user.id).maybeSingle();
 if (m2) {
 const xp = (m2.xp_total || 0) + ach.xp_reward;
 await supabase.from('experiment_metrics').update({ xp_total: xp, level: levelForXp(xp) }).eq('user_id', user.id);
 }
 }
 }
 },
