...
import { X, ArrowRight, ArrowLeft, Check, Repeat, Calendar, Clock, Sparkles, Paperclip, Trash2, Link as LinkIcon, CalendarDays, BriefcaseBusiness, CalendarRange, RotateCw, SlidersHorizontal, type LucideIcon } from 'lucide-react';
...
const QUICK_PRESETS: { id: Frequency; icon: LucideIcon; label: string; sub: string }[] = [
 { id: 'daily', icon: CalendarDays, label: 'Todos los días', sub: 'Lun a Dom' },
 { id: 'weekdays', icon: BriefcaseBusiness, label: 'Días laborables', sub: 'Lun a Vie' },
...
const QuickRecurrenceFlow = ({ open, onClose }: QuickRecurrenceFlowProps) => {
 ...
 frequency === p.id? "bg-primary text-primary-foreground border-primary shadow-xl shadow-primary/20 scale-[1.02]": "bg-surface border-border hover:border-primary/30 hover:bg-primary/5"
 )}
 >
 <PresetIcon className={cn(
 "h-6 w-6 mb-3",
 frequency === p.id ? "text-primary-foreground" : "text-primary"
 )} />
 <div className={cn(
 ...
 selectedDays.includes(d.value)? "bg-primary text-primary-foreground shadow-lg shadow-primary/20 scale-110": "text-muted-foreground hover:bg-black/5"
 )}
 >
 {d.label}
 </button>
 ...
 monthDay === day? "bg-primary text-primary-foreground shadow-md shadow-primary/20 scale-110": "text-muted-foreground hover:bg-black/5"
 )}
 >
 {day}
 </button>
 ...
 customFreq === f? "bg-background text-primary shadow-sm": "text-muted-foreground hover:bg-black/5"
 )}
 >
 ...
 {customFreq === 'daily'? (interval === 1? 'día': 'días'): customFreq === 'weekly'? (interval === 1? 'semana': 'semanas'): (interval === 1? 'mes': 'meses')}
 </span>
 </div>
 </div>
 )}
 ...
 estimatedMinutes === m? "bg-primary border-primary text-primary-foreground shadow-lg shadow-primary/20 scale-105": "bg-surface border-border text-foreground hover:border-primary/50"
 )}
 >
 {m >= 60? `${m/60}h`: `${m}m`}
 </button>
 ...
 hasSpecificTime? "bg-primary text-primary-foreground border-primary shadow-xl shadow-primary/20": "bg-surface border-border hover:border-primary/30"
 )}
 >
 <div className="flex items-center gap-4">
 <div className={cn(
 ...
 canAdvance? "bg-primary text-primary-foreground shadow-2xl shadow-primary/30 hover:scale-[1.02] active:scale-95": "bg-surface text-muted-foreground/30 border border-border"
 )}
 >
 {saving? (
